...
BinarySearchTree<Key, Value>::iterator::operator++() {
  ...
  Node<Key, Value> *next =
      BinarySearchTree<Key, Value>::successor(this->current_);
  ...
  while (temp != NULL) {
    if (keyValuePair.first < temp->getKey()) {
      if (temp->getLeft() == NULL) {
        ...
        Node<Key, Value> *node_l =
            new Node<Key, Value>(keyValuePair.first, keyValuePair.second, temp);
        temp->setLeft(node_l);
        return;
      }
      temp = temp->getLeft();
...
void BinarySearchTree<Key, Value>::remove(const Key &key) {
  ...
  /*if 2 children*/
    ...
    /*predecessor is either temp's left child or always a right child of a
     * parent*/
