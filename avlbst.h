 ...
 */
template <class Key, class Value> int AVLNode<Key, Value>::getHeight() const {
  return height_;
}
...
protected:
  ...
  void pre_rotate(AVLNode<Key, Value> *g, AVLNode<Key, Value> *p,
                  AVLNode<Key, Value> *n);
...
template <class Key, class Value>
void AVLTree<Key, Value>::update_height(AVLNode<Key, Value> *up,
                                        AVLNode<Key, Value> *down) {

  /*now up is the lower node*/
  if (up->getLeft() == NULL && up->getRight() == NULL) {
    ...
    int up_now =
        std::max(up->getLeft()->getHeight(), up->getRight()->getHeight()) + 1;
    up->setHeight(up_now);
  }
  ...
  if ((down->getRight() == NULL) && (down->getLeft() != NULL)) {
    ...
    /*both children exists*/
    int down_now =
        std::max(down->getLeft()->getHeight(), down->getRight()->getHeight()) +
        1;
...
template <class Key, class Value>
void AVLTree<Key, Value>::left_rotate(AVLNode<Key, Value> *up,
                                      AVLNode<Key, Value> *down) {
...
template <class Key, class Value>
void AVLTree<Key, Value>::right_rotate(AVLNode<Key, Value> *up,
                                       AVLNode<Key, Value> *down) {
  /*Because root node's parent is not root_ */
  bool up_root = false;
  if (this->root_ == up) {
...
template <class Key, class Value>
void AVLTree<Key, Value>::pre_rotate(AVLNode<Key, Value> *g,
                                     AVLNode<Key, Value> *p,
                                     AVLNode<Key, Value> *n) {
...
void AVLTree<Key, Value>::insert_fix(AVLNode<Key, Value> *p,
  ...
  if (g->getLeft() == NULL) {
    ...
    /*g has both children*/
    gh_now =
        std::max(g->getLeft()->getHeight(), g->getRight()->getHeight()) + 1;
    g_status = 2;
  }
  ...
  while (temp != NULL) {
    if (new_item.first < temp->getKey()) {
      if (temp->getLeft() == NULL) {
        ...
        AVLNode<Key, Value> *node_l =
            new AVLNode<Key, Value>(new_item.first, new_item.second, temp);
        temp->setLeft(node_l);
...
void AVLTree<Key, Value>::remove_fix(AVLNode<Key, Value> *n) {
  ...
  if ((n->getLeft() == NULL) && (n->getRight() == NULL)) {
    ...
    /*n has both children*/
    nh_now =
        std::max(n->getLeft()->getHeight(), n->getRight()->getHeight()) + 1;
    n_status = 3;
  }
...
void AVLTree<Key, Value>::remove(const Key &key) {
  ...
  /*if 2 children*/
    ...
    a right child would have been the predecessor*/
      ...
      if (pre->getKey() == temp->getParent()->getKey()) {
        /*pre is the parent, temp is a left child*/
        temp->getParent()->setLeft(temp->getLeft());
      } else {
        /*pre is upper than parent, temp is the rightest child*/
        temp->getParent()->setRight(temp->getLeft());
      }
    }
  }
